...
class ObjFile(object):
    ...
    def read(self):
        ...
        # Python3: map() objects must be materialized into lists
        if len(verts) > 0 and isinstance(verts[0], map):
            verts = [list(v) for v in verts]
        if len(faces) > 0 and isinstance(faces[0], map):
            faces = [list(f) for f in faces]
        if norms is not None and len(norms) > 0 and isinstance(norms[0], map):
